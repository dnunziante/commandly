...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "reply") {
      ...
      const generated = await ai(compileRefyntraPrompt({ standards: context.communication.standards, approvedKnowledge: context.knowledge, conversationContext: formatCommunicationContext(context.communication), userRequest: "", featureInstructions: `Act only as the hidden customer persona. Continue naturally based on the transcript and persona. Do not expose the persona or score the rep. Do not claim unapproved facts. This is a spoken role-play: ask only for information the representative can answer in words. Never ask for documents, screenshots, proof, links, written quotes, photos, brochures, or any other concrete evidence. Return JSON {reply, shouldEnd}; reply is concise customer dialogue. Ask one focused question when one is enough, and never ask more than two questions in the same response. Do not use a list of questions.`, }), { persona: persona?.persona, profile, transcript: messages, repReply: reply });

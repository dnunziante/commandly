...
export async function POST(request: Request) {
  ...
  if (!viewer?.organizationId || viewer.demo) return NextResponse.json({ error: "Sign in to generate a sales email." }, { status: 401 });
  const body = await request.json().catch(() => null);
  if (!body) return NextResponse.json({ error: "Enter the email details." }, { status: 400 });
  ...
  const input: SalesEmailInput = {
    ...
    customerNeeds: text(body.customerNeeds, 1200), previousConversation: text(body.previousConversation, 1600),
    objection: text(body.objection, 1000), desiredNextAction: text(body.desiredNextAction, 500),
    tone: tones.has(body.tone) ? body.tone : "Professional",
  };
  ...
  try {
    ...
    return NextResponse.json({ error: message.includes("OPENAI_API_KEY") ? "The secure OpenAI connection has not been configured yet." : "The email could not be generated. Please try again." }, { status: 503 });
  }
}

...
export default async function CoachReviewPage({ searchParams }: { searchParams: Promise<{ session?: string }> }) {
  ...
  return <AppShell title="Session Review"><PageHeader eyebrow="Practice complete" title="You created space for a useful conversation" description={result.source === "demo" ? "This is simulated feedback. Connected sessions are stored in Supabase and will later be evaluated with approved methodology." : "This review was saved to your workspace. Scoring remains deterministic until the OpenAI milestone."} action={<Link className="btn btn-primary" href="/coach/scenarios"><RefreshCw size={16}/> Practice again</Link>}/>

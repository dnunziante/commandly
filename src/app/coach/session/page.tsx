...
export default async function CoachSessionPage({ searchParams }: { searchParams: Promise<{ scenario?: string }> }) {
  ...
  return <AppShell title="Role-Play Session"><PageHeader eyebrow={`${scenario.difficulty} · ${scenario.duration}`} title={scenario.title} description={`You are speaking with ${scenario.customer.toLowerCase()}. Choose the response that best advances a helpful, consultative conversation.`}/><CoachSession scenario={scenario}/></AppShell>;
}

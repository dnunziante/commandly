...
export function AdminProductForm({ families }: { families: ProductFamilyDTO[] }) {
  ...
  return <form ref={formRef} className="card form-stack" onSubmit={handleSubmit}>
    ...
    {!isAddOn && <><div className="grid grid-2"><div><label className="label" htmlFor="range">Range</label><input className="input" id="range" name="range" placeholder="Up to 55 mi"/></div><div><label className="label" htmlFor="seats">Capacity</label><input className="input" id="seats" name="seats" placeholder="4 passengers"/></div></div>

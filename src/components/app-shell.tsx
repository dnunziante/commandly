    ...
    <aside className={`sidebar ${open ? "sidebar-open" : ""}`}>
      ...
      <nav className="nav module-nav" aria-label="Main navigation">
        ...
        {executiveVisible && <details className="module-group" key={`executive-${pathname}`} open={executiveSectionActive}>
          <summary><span className="module-icon"><Crown size={18}/></span><span>Executive Advisor</span><span className="live-badge">Prototype</span><ChevronDown className="module-chevron" size={16}/></summary>
      ...
      <div className="sidebar-footer">{administratorVisible && <Link href="/admin/settings"><Settings size={18}/> Settings</Link>}<div className="profile"><span className="avatar">{viewer.initials}</span><span><strong>{viewer.fullName}</strong><small>{roleLabel}</small></span>{viewer.demo ? <span className="demo-mode-label">Local demo</span> : <form action={logout}><button className="logout-button" type="submit">Sign out</button></form>}</div></div>
    </aside>
    <main className="main">

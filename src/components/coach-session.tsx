...
import type { CoachLocationAccess } from "@/lib/coach/access";
import type { CoachScenario } from "@/lib/coach/types";

export function CoachSession({ scenario, locationAccess }: { scenario: CoachScenario; locationAccess: CoachLocationAccess }) {
  ...
  const [locationId, setLocationId] = useState(locationAccess.selectedLocationId ?? "");
  ...
  return <div className="coach-session-grid">
    ...
    <aside className="coach-side-stack" aria-label="Coaching panel">
      <section className="card"><span className="badge"><Sparkles size={13}/> Practice objective</span><h2>{scenario.title}</h2><p>{scenario.goal}</p>{locationAccess.locations.length > 0 && (locationAccess.canSelectLocation ? <label><span className="label">Location</span><select className="input" value={locationId} onChange={(event) => setLocationId(event.target.value)}>{locationAccess.locations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</select></label> : <p><strong>Practice location:</strong> {locationAccess.locations[0].name}</p>)}{locationAccess.error && <p className="form-error" role="alert">{locationAccess.error}</p>}<div className="chips">{round.skillImpacts.map((skill) => <span className="chip" key={skill}>{skill}</span>)}</div></section>
      <section className={`card coach-feedback ${submitted ? "ready" : ""}`} aria-live="polite">
        {submitted ? <><CheckCircle2 size={27}/><h2>Round feedback</h2><p>{preferred ? "Strong response. You kept the conversation consultative and advanced the skills highlighted for this round." : "You moved forward before completing enough discovery. Slow down and invite the customer to clarify their priorities."}</p><strong>Skills evaluated</strong><p>{round.skillImpacts.join(" · ")}</p>{error && <p className="form-error" role="alert">{error}</p>}<button className="btn btn-primary" disabled={isPending || Boolean(locationAccess.error)} onClick={continueSession}>{isPending ? <><LoaderCircle className="spin" size={16}/> Saving session...</> : finalRound ? <>Complete session <ArrowRight size={16}/></> : <>Continue to round {roundIndex + 2} <ArrowRight size={16}/></>}</button></> : <><MessageCircle size={27}/><h2>Feedback appears here</h2><p>Select and submit a response. Your final score combines all rounds using this scenario&apos;s C.L.O.S.E.R. weights.</p></>}
      </section>
    </aside>
  </div>;
}

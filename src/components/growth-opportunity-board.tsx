...
export function GrowthOpportunityBoard({ opportunities, weights }: { opportunities: GrowthOpportunity[]; weights: GrowthScoreWeights }) {
  ...
  const visible = useMemo(() => opportunities.filter((item) => (category === "All opportunities" || item.category === category) && (status === "All statuses" || (item.lifecycleStatus || "idea") === status) && `${item.title} ${item.summary}`.toLowerCase().includes(query.toLowerCase())).sort((a, b) => sort === "Priority score" ? calculateGrowthPriority(b.score, weights) - calculateGrowthPriority(a.score, weights) : a.title.localeCompare(b.title)), [category, opportunities, query, sort, status, weights]);
  return <>
    <section className="card growth-filter" aria-label="Opportunity filters">
      ...
      <label><span className="label">Status</span><select className="input" value={status} onChange={(event) => setStatus(event.target.value)}><option>All statuses</option><option value="idea">Idea</option><option value="under_review">Under review</option><option value="approved">Approved</option><option value="in_progress">In progress</option><option value="blocked">Blocked</option><option value="completed">Completed</option></select></label>
    ...
    {visible.length ? <div className="grid grid-3 growth-grid">{visible.map((item) => <article className="card growth-card" key={item.slug}>
      ...
      <small>{item.lifecycleStatus ? `${item.lifecycleStatus.replaceAll("_", " ")} · ${item.progress ?? 0}% complete` : "Idea · 0% complete"}</small>

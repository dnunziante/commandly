...
export function ProductEditor({ product, families, organizationId }: { product: ProductDTO; families: ProductFamilyDTO[]; organizationId: string }) {
  ...
  return <form className="card form-stack product-edit-form" onSubmit={submit}>
    ...
    <div className="grid grid-3"><div><label className="label" htmlFor="price">Starting price</label><input className="input" id="price" name="price" type="number" min="0" step="0.01" required defaultValue={product.price}/></div><div><label className="label" htmlFor="range">Range</label><input className="input" id="range" name="range" defaultValue={product.range}/></div><div><label className="label" htmlFor="seats">Capacity</label><input className="input" id="seats" name="seats" defaultValue={product.seats}/></div></div>

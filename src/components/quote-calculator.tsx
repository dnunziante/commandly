...
const initialQuote: QuoteInputs = { vehiclePrice: 0, accessories: 0, docFees: 0, tradeIn: 0, discount: 0, salesTax: 0, extendedWarranties: 0, tagTitleDmvFee: 0, shippingDestination: 0, delivery: 0 };
const labels: Array<[keyof QuoteInputs, string, boolean]> = [
  ...
  ["shippingDestination", "Shipping / Destination", false], ["delivery", "Delivery", false],
];
...
export function QuoteCalculator({ vehicles, accessories, warranties, locations }: { vehicles: ProductDTO[]; accessories: ProductDTO[]; warranties: ProductDTO[]; locations: OrganizationLocation[] }) {
  ...
  const [overriddenFees, setOverriddenFees] = useState({ tax: false, shipping: false, delivery: false });
  ...
  return <section className="card quote-calculator">
    <div className="metric-row quote-heading"><div><h2>Quote details</h2><p>Enter each amount to calculate the delivered total.</p></div><button className="btn btn-ghost" type="button" onClick={() => { setQuote(initialQuote); setVehicleId("manual"); setSelectedAccessoryIds([]); setSelectedWarrantyIds([]); setLocationId("manual"); setOverriddenFees({ tax:false, shipping:false, delivery:false }); }}><RotateCcw size={15}/> Reset</button></div>
    <div className="quote-sheet">
      <div className="quote-location"><label className="label" htmlFor="quote-location">Dealership Location</label><select className="input" id="quote-location" value={locationId} onChange={(event) => { const id = event.target.value; setLocationId(id); const location = locations.find((item) => item.id === id); if (location) { setQuote((current) => ({ ...current, shippingDestination: location.shippingDestinationFee, delivery: location.deliveryFee })); setOverriddenFees({ tax:false, shipping:false, delivery:false }); } }}><option value="manual">Manual fees</option>{locations.map((location) => <option value={location.id} key={location.id}>{location.name}</option>)}</select></div>
      ...
      <label className="quote-row"><span>Shipping / Destination:{overriddenFees.shipping && <small className="quote-override">Override</small>}</span><span className="quote-input-wrap"><span>$</span><input aria-label="Shipping / Destination" type="number" min="0" step="0.01" value={quote.shippingDestination} onChange={(event) => { update("shippingDestination", event.target.value); if (locationId !== "manual") setOverriddenFees((current) => ({...current,shipping:true})); }}/></span></label>

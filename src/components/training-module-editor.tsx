...
export function TrainingModuleEditor({ lessons, module, initialLessonIds = [] }: { lessons: TrainingLessonDTO[]; module?: TrainingModuleDTO; initialLessonIds?: string[] }) {
  ...
  return <form className="card form-stack training-module-editor" action={action}>
    ...
    <div className="metric-row"><div><h2>{module ? module.title : "Create a training module"}</h2><p>{module ? "Edit its details, knowledge-backed lessons, and order." : "Select one or more knowledge uploads to create a larger learning path."}</p></div>{module ? <div className="training-module-heading-actions"><span className={`badge ${module.isPublished ? "blue" : "amber"}`}>{module.isPublished ? "Published" : "Draft"}</span><DeleteTrainingModuleButton moduleId={module.id} title={module.title}/></div> : null}</div>
    ...
    })}</div><small className="field-help">Check every knowledge upload to include. Uncheck an item and save to remove it from this module; its original Knowledge Base upload and lesson are kept. Lower order numbers appear first.</small></fieldset>

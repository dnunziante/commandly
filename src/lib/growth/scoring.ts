...
export async function getGrowthScoring() {
  ...
  if (!viewer || viewer.demo) return { opportunities: growthOpportunities, weights: growthScoreWeights as GrowthScoreWeights, persistence: "demo" as const, canEdit: true, error: "" };
  ...
  const persisted = (opportunityRows ?? []).map((row) => ({ slug: row.slug, title: row.title, category: row.category, summary: row.summary, rationale: row.rationale, impact: row.impact_label, effort: row.effort_label, timeframe: row.timeframe, status: "Ready to review" as const, actions: row.actions, measures: row.measures, score: scores[row.slug] ?? row.score })) as typeof growthOpportunities;
...
export async function getAdminGrowthOpportunities() {
  const viewer = await getViewer(); if (!viewer || viewer.demo) return { opportunities: growthOpportunities.map((item)=>({...item,adminStatus:"published"})), persistence:"demo" as const, canEdit:true, error:"" };

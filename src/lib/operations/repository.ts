...
export async function getOperationsWorkspace(): Promise<OperationsWorkspace> {
  ...
  const [checklistsResult, proceduresResult, alertsResult, schedulesResult, handoffsResult, incidentsResult] = await Promise.all([
    ...
    supabase.from("operations_procedures").select("id,title,category,owner,summary,status,version,updated_at,operations_procedure_steps(id,title,position)").eq("organization_id", organizationId).neq("status", "archived").order("updated_at", { ascending: false }),
  ...
  const firstError = [checklistsResult, proceduresResult, alertsResult, schedulesResult, handoffsResult, incidentsResult].find((result) => result.error)?.error;

  type ChildStep = { id: string; title: string; position: number };
  const procedures = (proceduresResult.data ?? []).map((row) => ({
    id: row.id, title: row.title, category: row.category as OperationsProcedureRecord["category"], owner: row.owner,
